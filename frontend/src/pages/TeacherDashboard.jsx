...
import { Row, Col, Card, Statistic, Table, Tag, Typography, Spin, Progress, Alert, Tabs } from 'antd';
...
import { TeamOutlined, BookOutlined, WarningOutlined, TrophyOutlined, RiseOutlined } from '@ant-design/icons';
...
export default function TeacherDashboard() {
  ...
  const loadData = async () => {
    try {
      const res = await dashboardAPI.getSummary(DEMO_COURSE_ID);
  ...
  const errorBarOption = {
    ...
    xAxis: {
      type: 'category',
      data: summary.common_errors.map(e => ({
        knowledge: '知识型', method: '方法型', calculation: '计算型', metacognition: '元认知型'
      }[e.type] || e.type))
    },
    yAxis: { type: 'value' },
    series: [{
      type: 'bar',
      data: summary.common_errors.map(e => e.count),
  ...
  const columns = [
    ...
    {
      title: '掌握度', dataIndex: 'avg_mastery', key: 'avg_mastery',
      render: (v) => <Progress percent={Math.round(v)} size="small" />,
    },
    {
      title: '预警', dataIndex: 'alert_level', key: 'alert_level',
      render: (v) => {
        const colors = { none: 'green', blue: 'blue', yellow: 'orange', red: 'red' };
        return <Tag color={colors[v] || 'green'}>{v === 'none' ? '正常' : `${v}级预警`}</Tag>;
      },
    },
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}>
          <Card title="🚨 预警分布" size="small">
            <ReactEChartsCore echarts={echarts} option={alertPieOption} style={{ height: 220 }} />
          </Card>
        </Col>
        <Col span={8}>
          <Card title="📈 共性错误分布" size="small">
            <ReactEChartsCore echarts={echarts} option={errorBarOption} style={{ height: 220 }} />
          </Card>
        </Col>
        <Col span={8}>
          <Card title="💡 教学建议" size="small" style={{ height: 270 }}>
            <Alert
              ...
              showIcon
              message="薄弱知识点"
              description="全班42%学生对「极限定义」掌握不足，建议安排专题复习课。"
            ...
            <Alert
              ...
              showIcon
              message="分层教学"
              description="建议按掌握度将学生分为3组，分别推送差异化练习。"
            />
          </Card>
        </Col>
      </Row>
